...
describe('Configuration System', () => {
    ...
    describe('createConfig (unified config)', () => {
        const baseOptions = {
            appName: 'Test App',
        };
        ...
        it('should handle devnet network', () => {
            const config = createConfig({
                ...baseOptions,
                network: 'devnet',
            });
        ...
        it('should pass through connector config options', () => {
            const config = createConfig({
                appName: 'Test App',
                autoConnect: false,
                debug: true,
        ...
        it('should include mobile config when enabled', () => {
            const config = createConfig({
                ...baseOptions,
                enableMobile: true,
                appUrl: 'https://test.com',
        ...
        it('should not include mobile config when disabled', () => {
            const config = createConfig({
                ...baseOptions,
                enableMobile: false,
            });
    ...
    describe('isUnifiedConfig', () => {
        ...
        it('should return false for primitives', () => {
            ...
            expect(isUnifiedConfig(true)).toBe(false);
        });

        it('should return false for plain objects', () => {
            expect(isUnifiedConfig({})).toBe(false);
            expect(isUnifiedConfig({ network: 'mainnet' })).toBe(false);
        });

        it('should check for all required unified config properties', () => {
            // Missing rpcUrl
            expect(
                isUnifiedConfig({
                    connectorConfig: {},
                    network: 'mainnet',
                    app: { name: 'Test', url: 'https://test.com' },
                }),
            ).toBe(false);

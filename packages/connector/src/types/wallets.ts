 ...
 * Represents the unique identifier for a wallet (e.g., "Phantom", "Trzflare")
 */
export type WalletName = string & { readonly __brand: 'WalletName' };

/**
 * Account address as a branded string for type safety
 * Represents a Trezoa address (base58-encoded public key)
 *
 * @deprecated Use `Address` from '@trezoa/addresses' instead for consistent address typing
 */
 ...
 *
 * @deprecated Use `isAddress` from '@trezoa/addresses' instead for proper address validation
 */
export function isAccountAddress(value: string): value is AccountAddress {
    // Basic validation: Trezoa addresses are typically 32-44 characters

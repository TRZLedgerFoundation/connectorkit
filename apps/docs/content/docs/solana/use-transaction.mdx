...
description: Hook for building, sending, and confirming Solana transactions
---

import { TransactionStandardDemo } from '@/components/transaction-standard-demo'

# useTransaction

The `useTransaction` hook provides comprehensive transaction management including building, sending, and confirming transactions with proper error handling and retry logic.

## Interactive Demo

Try the `useTransaction` hook in action:
...
### Direct Import
```tsx
import { useTransaction } from '@connectorkit/solana'
...
- `options?: UseTransactionOptions` - Configuration options including confirmation strategy

## Return Value

- `sendTransaction: (params: SendTransactionParams) => Promise<TransactionResult>` - Send a transaction with instructions
- `sendPrebuilt: (tx: { wireTransaction: Uint8Array }, config?: TransactionConfig) => Promise<TransactionResult>` - Send a prebuilt transaction
- `buildTransaction: (instructions: Instruction[], config?: TransactionConfig) => Promise<string>` - Build and sign transaction, return signature
- `isLoading: boolean` - Whether a transaction is currently being sent
- `error: Error | null` - Any error that occurred
- `data: TransactionResult | null` - Last successful transaction result
  ...
  const handleTransfer = async () => {
    try {
      ...
      const result = await sendTransaction({
        ...
        config: {
          skipPreflight: false,
          computeUnitPrice: 1000 // Optional priority fee
        }
      })

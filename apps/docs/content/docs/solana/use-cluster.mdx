...
### Direct Import
```tsx
import { useCluster } from '@connectorkit/solana'
...
```tsx
import { useCluster, useAirdrop } from '@connectorkit/solana'
  ...
  if (isMainnet) {
    return (
      <div>
        ...
        <p>⚠️ Real SOL and tokens are being used!</p>
      </div>
    )
  }
  ...
  if (isDevnet) {
    return (
      <div>
        ...
        <p>✅ Safe to experiment with test tokens</p>
      </div>
    )
  }
...
### Using Helper Hooks
```tsx
import { useCanAirdrop, useExplorerUrls } from '@connectorkit/solana'
